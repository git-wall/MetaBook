...
public class Rotation3DSample {
    public static void play(TextSurface textSurface) {
        Text textA = TextBuilder.create("You want to chat ?").setSize(25).setColor(Color.BLACK).setPosition(Align.SURFACE_CENTER).build();
        Text textB = TextBuilder.create("Make more friend ?").setSize(25).setColor(Color.BLACK).setPosition(Align.SURFACE_CENTER, textA).build();
        Text textC = TextBuilder.create("Let go!").setSize(25).setColor(Color.BLACK).setPosition(Align.SURFACE_CENTER, textB).build();
        int duration = 1510;
